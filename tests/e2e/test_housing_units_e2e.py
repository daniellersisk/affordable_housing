...
def test_create_read_update_delete_lifecycle(client: TestClient, auth_headers: dict) -> None:
    ...
    # read back
    fetched = client.get(f"/housing-units/{unit_id}")
    ...
    # delete
    deleted = client.delete(f"/housing-units/{unit_id}", headers=auth_headers)
    assert deleted.status_code == 204

    # confirm gone
    gone = client.get(f"/housing-units/{unit_id}")
...
def test_list_filters_by_borough(client: TestClient, auth_headers: dict) -> None:
    ...
    response = client.get("/housing-units?borough=BRONX")
...
def test_list_filters_by_num_units_range(client: TestClient, auth_headers: dict) -> None:
    ...
    client.post("/housing-units", json={"num_units": 5}, headers=auth_headers)
    client.post("/housing-units", json={"num_units": 50}, headers=auth_headers)
    client.post("/housing-units", json={"num_units": 500}, headers=auth_headers)

    response = client.get("/housing-units?num_units_min=10&num_units_max=100")
    assert response.status_code == 200
    items = response.json()["items"]
...
def test_unauthenticated_write_is_rejected(client: TestClient) -> None:
    ...
    assert client.post("/housing-units", json={"num_units": 1}).status_code == 401
    assert client.put("/housing-units/1", json={"num_units": 1}).status_code == 401
    assert client.delete("/housing-units/1").status_code == 401


@pytest.mark.e2e
def test_get_endpoints_are_public(client: TestClient) -> None:
    """GET endpoints return 200 without auth header."""
    assert client.get("/housing-units").status_code == 200
    assert client.get("/housing-units/999999999").status_code == 404
...
def test_pagination(client: TestClient, auth_headers: dict) -> None:
    ...
    for i in range(6):
        client.post("/housing-units", json={"num_units": i + 1}, headers=auth_headers)

    page1 = client.get("/housing-units?limit=3&offset=0").json()["items"]
    page2 = client.get("/housing-units?limit=3&offset=3").json()["items"]
